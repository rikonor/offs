# AI Agent Guidelines (AGENTS.md) - STRICT MODE
...
-   **Linting:** Code must pass `cargo clippy -- -D warnings`. NO WARNINGS
    ALLOWED.
-   **Documentation:** All public items MUST have documentation comments.
-   **Tests:** All new features MUST include unit tests.
-   **Error Handling:** Prefer `thiserror` for libraries and `anyhow` for
    applications. `unwrap()` is FORBIDDEN in production code.
